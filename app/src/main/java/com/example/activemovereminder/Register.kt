...
class Register : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        passEt = findViewById(id.passEt)
        nameEt = findViewById(id.nameEt)
        dateTxt = findViewById(id.dateTxt)
        register = findViewById(id.register)
        auth = FirebaseAuth.getInstance()

        //Efectos de sonido
        soundPool = SoundPool.Builder().setMaxStreams(1).build()
        soundId = soundPool.load(this, raw.menu, 1)
    ...
    private fun updateUI(user: FirebaseUser?) = //hi ha un interrogant perquè podria ser null
        if (user != null) {
            ...
            val reference: DatabaseReference = database.getReference("DATA_BASE_JUGADORS_AMR")
            if(reference!=null) {
